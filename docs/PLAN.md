...
| 06 | **Reopened 2026-09-20.** `src/cvops/api/app.py`: thin FastAPI wrapping the exact `services/` calls the CLI makes, no logic duplicated. `frontend/`: Next.js 16 web app -- dashboard, resume upload and review/edit, builder, and the original tabbed tool. Design: Glassmorphism, dark (`design-system/`); animation building blocks in `frontend/src/lib/fx/`. Dependencies added since the first cut: `motion` (animation) and `three` (lazy-loaded, one particle effect). | Ravi asked for it directly -- see 01. | Auth, multi-user, or a deploy target would need real design work; today it's `localhost:8000` <-> `localhost:3000`, one person, one machine. |
