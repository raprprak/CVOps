...
## Status
P0-P3 done (2026-09-15): CLI project flattened to the repo root, Typst chosen, plan written (P0). Pydantic models, resolve.py, Typst render pipeline, `cvops build`/`show` (P1). `services/ats_lint.py` (rules L1-L10, two independent text extractors: pdftotext + pdfplumber), `cvops lint`, GitHub Actions CI (P2). `services/match.py` (exact-string JD keyword coverage: known master skills incl. aliases + a heuristic pass for tech-looking tokens not yet in master.yaml; required/preferred section weighting), `services/tailor.py` (proposes a target by selecting/ordering master ids by tag overlap -- never writes an override), `cvops match`/`tailor` (P3). `docs/ats-field-tests.md` scaffolded for the ground-truth ATS-portal log (2026-09-17).

Real `uv.lock` + a working `.venv` now exist (built 2026-09-15 via a real `uv sync --all-groups` on the developer's own Mac -- typst 0.15.0, pytest 9.1.1, ruff 0.16.7, pydantic 2.13.5, pypdf 6.18.1, pdfplumber 0.11.10, typer 0.27.2, cvops installed editable). The assistant re-verified `render.py`'s `typst.compile(...)` call and `ats_lint.py`'s pypdf/pdfplumber usage line-by-line against these exact installed packages' real signatures (read from `.venv/lib/.../site-packages` on the developer's machine) -- no mismatches found. Neither of the assistant's two shells (its own cloud sandbox, and the sandboxed `device_bash` VM on the developer's Mac) can execute this `.venv` (its Python interpreter is a macOS arm64 binary) or reach PyPI/GitHub to build their own, so `pytest`/`cvops build --all`/`cvops lint --all`/`ruff check` have still never actually been run -- that can only happen in the developer's own real Terminal, where the environment is now fully ready. Remaining per docs/PLAN.md: run those for real, install graphify (`.claude/rules/context.md`), fill in `docs/ats-field-tests.md` with real portal submissions, replace the placeholder `data/master.yaml`/`data/targets/example.yaml` with real career data. P4 (watch mode, DOCX, LLM-assisted rewriting, thin API) is deliberately deferred.
