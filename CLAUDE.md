...
## Status
P0-P3 done (2026-09-15): CLI project flattened to the repo root, Typst chosen, plan written (P0). Pydantic models, resolve.py, Typst render pipeline, `cvops build`/`show` (P1). `services/ats_lint.py` (rules L1-L10, two independent text extractors: pdftotext + pdfplumber), `cvops lint`, GitHub Actions CI (P2). `services/match.py` (exact-string JD keyword coverage: known master skills incl. aliases + a heuristic pass for tech-looking tokens not yet in master.yaml; required/preferred section weighting), `services/tailor.py` (proposes a target by selecting/ordering master ids by tag overlap -- never writes an override), `cvops match`/`tailor` (P3). 49 unit tests passing against pydantic/jinja2/pypdf/pdfplumber/reportlab already available; not run in-session: `uv sync`, real `typst` compiles, `pytest`/`ruff` via uv (PyPI was unreachable from the assistant's shells -- see chat). Remaining per docs/PLAN.md: run the P0-P3 exit criteria for real (`uv sync`, `cvops build --all`, `cvops lint --all`, `pytest`), install graphify (`.claude/rules/context.md`), start the ground-truth field-test log. P4 (watch mode, DOCX, LLM-assisted rewriting, thin API) is deliberately deferred.
